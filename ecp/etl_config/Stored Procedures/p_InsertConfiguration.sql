CREATE PROCEDURE [etl_config].[p_InsertConfiguration] 
	@jobKey int 
AS
BEGIN
    ...
    BEGIN TRY
		...
		EXEC [etl_audit].[p_UpdateJobLog_EndSection] @section = @section, @jobLogKey = @jobLogKey OUTPUT;
			...
			WITH src AS (SELECT val.[TruncateDataWarehouse]
						,		val.[CalendarStartDate]
						FROM	(
								-- Minimum configuration to operate
										SELECT	'No'					,'1 Jan 2021'
								) val (			[TruncateDataWarehouse],[CalendarStartDate])
						)

			INSERT INTO [etl_config].[Configuration]
			...
			,		ISNULL(CAST(new.[CalendarStartDate]		AS date			), '1 jan 2021'	) AS [CalendarStartDate]			
			...
			WHERE NOT EXISTS (SELECT 1 FROM [etl_config].[Configuration]);
